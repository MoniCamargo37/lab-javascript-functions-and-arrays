...
function findLongestWord(names) {
  ...
  let longestWord = "";
  names.forEach(function (nameToCheck) {
    if (nameToCheck.length > longestWord.length) {
      longestWord = nameToCheck;
    }
  });
  return longestWord;
}
...
function sum(arrayOfThings) {
  ...
  for (let i = 0; i < arrayOfThings.length; i++) {
    if (typeof arrayOfThings[i] == "string") {
      suma += arrayOfThings[i].length;
    } else if (typeof arrayOfThings[i] == "number") {
      suma += arrayOfThings[i];
...
function averageWordLength(thing) {
  if (thing.length === 0) {
    return null;
  }
  let value = 0;
  thing.forEach(function (word) {
...
function doesWordExist(arrayOfWords, wordToSearch) {
  ...
  console.log(arrayOfWords, " ", wordToSearch);
...
function greatestProduct(products) {
  ...
  if (allOne === countElements) {
    return 1;
  }
  if (allTwo === countElements) {
    return 16;
  }
}
